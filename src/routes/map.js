...
/** Convert a PostGIS geometry (WKB hex or GeoJSON object/string) to a GeoJSON object */
function toGeoJSON(geom) {
  ...
  if (typeof geom === 'string') {
                  ...
                  // WKB hex from PostGIS
                try {
                                  return wkx.Geometry.parse(Buffer.from(geom, 'hex')).toGeoJSON();
                } catch (e) {
                                  return null;
                }
  }
...
router.get('/territories', async (req, res) => {
              try {
                ...
                const territories = (zones || []).map(z => {
                                  const geojson = toGeoJSON(z.geom);
                                  return {
                                                      zone_id: z.id,
                                                      runner_id: z.user_id,
...
router.get('/geojson', async (req, res) => {
              try {
                              ...
                              const features = (zones || []).map(z => {
                                                const geometry = toGeoJSON(z.geom);
                                                return {
                                                                    type: 'Feature',
                                                                    properties: {
                                                                                          zone_id: z.id,
